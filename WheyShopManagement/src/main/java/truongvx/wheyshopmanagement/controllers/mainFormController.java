...
public class mainFormController  implements Initializable {
  ...
  public void dashboardDisplayTI() {
    ...
    try {
      ...
      if (result.next()) {
        ti = result.getDouble(1); // Sử dụng chỉ số cột thay vì tên cột
      }
      System.out.println("Today's income: $" + ti); // In ra giá trị để kiểm tra
  ...
  public void dashboardTotalI() {
    ...
    try {
      ...
      if (result.next()) {
        ti = result.getFloat(1); // Sử dụng chỉ số cột thay vì tên cột
      }
      System.out.println("Total income: $" + ti); // In ra giá trị để kiểm tra
